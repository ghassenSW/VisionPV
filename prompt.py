...
6. **Causes de sinistre** : (LOGIQUE PAR ÉTAPES) :
    ...
    * LISTE OBLIGATOIRE (Sélectionnez une seule valeur exacte) :  
        ...
        Route glissante
7. **Nom du poste / Délégation** : Identifiez le nom géographique principal (la ville ou région) en analysant les en-têtes administratifs situés en haut à droite de chaque page. Ce nom se trouve généralement à la fin d'une ligne hiérarchique, souvent précédé de la particule "ب" (en/à).
...
8. **Identification de l'Assurance** : Identifiez la compagnie d'assurance et transcrivez-la en utilisant uniquement les noms de la liste officielle ci-dessous.
...
9. **Sélection des victimes** : Incluez UNIQUEMENT les personnes ayant subi des dommages corporels. Vérifiez la section "الأضرار البدنية" et les compteurs "عدد الجرحى" / "عدد القتلى". Si une personne est indemne, ne l'ajoutez pas au JSON.
10. **Âge et Date de Naissance des victimes (RÈGLE OBLIGATOIRE)** : 
...
11. **Séparation des Noms et Prénoms** : Procédez à l'extraction sélective des noms et prénoms selon une logique binaire. Extrayez exclusivement le premier segment de l'identité complète pour le champ 'Prénom' et le dernier segment pour le champ 'Nom'.
...
12. **Catégorisation de l'État Social (MAPPING)** : 
...
13. **Type de véhicule (MAPPING)** : Vous devez impérativement faire correspondre le véhicule identifié à l'une des catégories suivantes, et UNIQUEMENT à celles-ci :
...
14. **Poste de Police / Garde Nationale** : Identifiez l'organisme ayant rédigé le PV en analysant l'en-tête (en haut à droite). Vous devez effectuer un choix binaire obligatoire :
...
- Type de véhicule : **CHOIX STRICT** dans la liste définie au point n°13. Ne créez aucune autre catégorie.
...
{{{{
    ...
    "N° Imm 1": "...",
    "Compagnie 1": "...",
    "Type de véhicule 1": "CHOIX STRICT LISTE POINT 13",
    ...
    "Age victime 1": 0,
    "Date naissance victime 1": "JJ/MM/AAAA",
