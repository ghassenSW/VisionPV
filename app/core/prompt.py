...
- **LOGIQUE DE DÉCISION PRIORITAIRE :**
    ...
    4. **Exception stricte pour la valeur officielle "Non mentionné / Non déterminé" :** Cette valeur fait partie de la liste officielle des causes possibles. Ne l'utilisez que si aucune autre cause n'est trouvée dans le bloc "أسباب الحادث" ni dans le contexte général du PV. Si au moins une autre cause est identifiable, ne retournez jamais "Non mentionné / Non déterminé" en plus d'une autre valeur et ne l'utilisez pas comme cause combinée.
...
LOGIQUE DE MAPPING :
ATTENTION À NE PAS CONFONDRE (CRUCIAL) :
- "التعاونية للتأمين للتعليم" (Education) correspond à MAE.
- "الصندوق التونسي للتأمين التعاوني الفلاحي" ou "تأمين فلاحي" (Agriculture) correspond à Groupe CTAMA.
...
"قات" / "GAT" → GAT
"كتاما" / "CTAMA" / "الصندوق التونسي للتأمين التعاوني الفلاحي" / "تأمين فلاحي" → Groupe CTAMA
"الليد" / "اللويد" / "LLOYD" → LLoyd Tunisien
"ت ت ت" / "تعاونية التأمين للتعليم" / "تعاونية التامين للتعليم" / "التعاونية للتأمين للتعليم" / "تعليم" / "MAE" → MAE
    ...
    **) Guide de Décision et Mots-Clés :
        ...
        - MAPPING ARABE → FRANÇAIS : Si le procès-verbal contient des termes arabes pour le type de véhicule,
            ...
            ou normalisation finale :
                "سيارة الأجرة" / "سيارة الأجرة الفردية" / "تاكسي" / "تاكسي فردي" / "تاكسي خاص" / "كرّاء تاكسي" -> "Taxi Individuel"
                "تاكسي جماعي" / "تاكسي مشترك" -> "Taxi collectif"
                "سيارة" / "عربة"                         -> "voiture"
        ...
        - GESTION DES ABSENCES : Si aucun indice clair ne permet d'identifier le type de véhicule dans le PV, retournez explicitement `null` pour ce champ. N'inventez jamais un type de véhicule et ne forcez pas une correspondance approximative.
    ...
    'Taxi collectif' : Utilisez cette valeur si le texte mentionne explicitement "taxi collectif", "taxi commun", "تاكسي جماعي" ou toute formulation équivalente de transport partagé.
